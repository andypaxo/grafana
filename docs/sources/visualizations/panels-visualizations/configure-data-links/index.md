...
1. In the dialog box that opens, define the API call settings:
   ...
   | Connection           | Specify how the action's HTTP request is sent. Choose from: **Direct from browser**, or routed through a configured data source if the `vizActionsAuth` feature toggle is enabled.                                                          |
